...
Returns a (suitably aligned for any built-in type) pointer to the allocated memory
...
If the system is out of memory, one or more processes will be killed by the OOM killer
...
the glibc malloc() allocates the memory as a private anonymous mapping using mmap(2)
...
In a multithreaded application in which threads simultaneously allocate and free 
...
Each arena is a large region of memory that is internally allocated by the system 
...
Crashes are almost always related to heap corruption, such as overflowing an allocated chunk or freeing the same pointer twice
*/
...
POSIX: the implementation shall support one or more programming environments where 
...
void* ft_calloc(size_t nmemb, size_t size)
{
  ...
  if (memory_to_return == NULL)
    return (NULL);
  ft_memset(memory_to_return, 0, nmemb * size);
