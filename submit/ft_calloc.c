...
void	*ft_calloc(size_t nmemb, size_t size)
{
	...
	if ((unsigned int)nmemb > INT_MAX / (unsigned int)size)
		return (NULL);
	printf("\n");
	printf("malloc(%zu * %zu = %zu = %d)\n",nmemb, size, nmemb * size, (unsigned int)nmemb * (unsigned int)size); //
	memory_to_return = (char *)malloc(nmemb * size);
	printf("sizeof(memory_to_return) = %zu\n",sizeof(memory_to_return)); //
